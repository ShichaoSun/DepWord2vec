...
int main(int argc, char **argv) {
    ...
    if ((i = ArgPos((char *)"-size", argc, argv)) > 0)
        depskgneg.Setlayer1_size(atoi(argv[i + 1]));


    if ((i = ArgPos((char *)"-debug", argc, argv)) > 0)
        depskgneg.SetDebugmode(atoi(argv[i + 1]));

    if ((i = ArgPos((char *)"-binary", argc, argv)) > 0)
        depskgneg.SetBinary(atoi(argv[i + 1]));

    //if ((i = ArgPos((char *)"-cbow", argc, argv)) > 0) cbow = atoi(argv[i + 1]);
    if ((i = ArgPos((char *)"-alpha", argc, argv)) > 0)
        depskgneg.SetAlpha(atof(argv[i + 1]));


    if ((i = ArgPos((char *)"-window", argc, argv)) > 0)
        depskgneg.SetWindow(atoi(argv[i + 1]));

    if ((i = ArgPos((char *)"-sample", argc, argv)) > 0)
        depskgneg.SetSample(atof(argv[i + 1]));

    //if ((i = ArgPos((char *)"-hs", argc, argv)) > 0) hs = atoi(argv[i + 1]);
    if ((i = ArgPos((char *)"-negative", argc, argv)) > 0)
        depskgneg.SetNegative(atoi(argv[i + 1]));

    if ((i = ArgPos((char *)"-threads", argc, argv)) > 0)
        depskgneg.SetNumthread(atoi(argv[i + 1]));
